...
write_bookings_to_csv(Date,Bookings, FilePath) ->
    ...
    ok = write_csv_row(File, ["Date","Booking ID", "User ID", "Name", "Email", "Category", "Price", "Ticket Count", "Total Price", "Total Tickets"]), % Write header row
...
write_booking_to_csv(Date,File, {bookings, BookingId, UserId, Name, Email, SeatsData, TotalPrice, TotalTickets}) ->
    lists:foreach(
        fun(Seats_data) ->
            ...
            write_csv_row(File, [Date,BookingId, UserId, String_name, String_email, Final, TotalPrice, TotalTickets])
        end,
        SeatsData
    ).

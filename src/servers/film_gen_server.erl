...
handle_call({login,{Email,Password}}, _From, State) ->
  ...
  Reply = case Data of
            ...
            [User]->
              Hashed_password=crypto:hash(sha256,Password),
              {users,Uid,Name,Email,Cur_password,
                Address,_Role,_Phone,User_name} = User,
...
initialize() ->
  ...
  mnesia:start(),
  mnesia:create_table(reminders, [{disc_copies,[node()]},{attributes, record_info(fields,reminders)},{type,set}]),
  mnesia:create_table(bookings, [{disc_copies,[node()]},{attributes, record_info(fields,bookings)},{type,set}]),
  mnesia:create_table(films, [{disc_copies,[node()]},{attributes, record_info(fields,films)},{type,set}]),
  mnesia:create_table(users, [{disc_copies,[node()]},{attributes, record_info(fields,users)},{type,set}]),
  mnesia:create_table(sequence,[{disc_copies,[node()]},{attributes, record_info(fields, sequence)}, {type, set}]),
...
ensure_user_sequence()->
      ...
      [] ->
        io:format("Key not found in sequence table. Initializing...~n"),
